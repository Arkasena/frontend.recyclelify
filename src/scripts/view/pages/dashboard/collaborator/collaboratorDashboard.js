...
const collaboratorDashboard = {
  async render() {
    return `
      <section class="flex pb-10 px-[32px] py-[25px] w-full bg-gray-50">
        <div class="grid grid-cols-2 gap-4 w-full">
            <div class="flex flex-col gap-2">
              <p class="text-lg font-medium">Transaksi Terakhir</p>
              <last-transaction></last-transaction>
            </div>
            <div class="flex flex-col gap-2">
              <p class="text-lg font-medium">Rekap Penjualan</p>
      ...
      lastTransactions.forEach((transaction) => {
        ...
        transactionItem.innerHTML = `
          <div class="flex flex-col gap-4 w-full h-full col-span-2 md:flex-row">
            <img src="${transaction.buyer.photo}" alt="Foto Profil Mitra" class="w-[50px] h-[50px] rounded-2xl">
            <div class="flex flex-col place-content-center">
              <p class="text-sm font-medium break-words">${transaction.buyer.name}</p>
