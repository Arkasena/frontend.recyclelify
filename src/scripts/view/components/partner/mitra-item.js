class MitraItem extends HTMLElement {
  ...
  render() {
    ...
    <img class="w-full aspect-video rounded-2xl " src="${this.mitraData.photo ? this.mitraData.photo : './images/others/blank-profile.png'}" alt="sampah">
    </div>
    <div class="flex flex-col px-6 py-4 shadow-sm">
    <h3 class="font-semibold text-lg mb-2">${this.mitraData.name}</h3>
    <p class="line-clamp-2 text-ellipsis mb-5">${this.mitraData.description ? this.mitraData.description : ''}</p>
    ...
    ${this.mitraData.material ? `
      ${this.mitraData.material.total > 1 ? `
        <div class="bg-lime-100 py-2 px-4 rounded-xl">${this.mitraData.material.name}</div>
        <div class="bg-lime-100 py-2 px-4 rounded-xl">+${Number(this.mitraData.material.total) - 1} lainnya</div>
      ` : `
        <div class="bg-lime-100 py-2 px-4 rounded-xl">${this.mitraData.material.name}</div>
      `}
    ` : `
